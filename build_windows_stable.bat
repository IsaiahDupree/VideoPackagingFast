@echo off
echo ===================================================
echo VideoPackagingFast - Windows Build Script (Stable)
echo ===================================================
echo.
...
REM Check if FFmpeg is in the path
where ffmpeg > nul 2>&1
if %ERRORLEVEL% EQU 0 (
    echo FFmpeg found in PATH, copying to ffmpeg_bin...
    for /f "tokens=*" %%i in ('where ffmpeg') do (
        copy "%%i" ffmpeg_bin\ffmpeg.exe
    )
) else (
